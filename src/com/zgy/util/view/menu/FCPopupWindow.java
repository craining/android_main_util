package com.zgy.util.view.menu;
...
public class FCPopupWindow {
	 ...
	 * @param anchor 显示位置参照点
	 * @param xoff  x方向偏移量 根据anchor
	 * @param yoff y方向偏移量 根据anchor
	 * @see: 
	 * @since: 
	 * @author: 李光辉 (ligh@35.cn)
	 * @date:2012-3-6
	 ...
	 * 
	 */
	public void showAtBottom(int widthLayoutParams, int heightLayoutParams, View contentView,
			View parentView, boolean isFocusable, BitmapDrawable bitmapDrawable) {
		if (isShowing()) {
			mPopupWindow.dismiss();
		}
		if (mPopupWindow == null) {
			mPopupWindow = new PopupWindow(contentView, widthLayoutParams, heightLayoutParams,
				isFocusable);
	...
	public void showAtBottom(View contentView) {
		showAtBottom(LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT, contentView,
			contentView.getRootView(), true, new BitmapDrawable());
	}

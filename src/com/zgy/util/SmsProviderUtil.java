...
import com.zgy.util.beans.SmsInfo;
...
public class SmsProviderUtil {
	...
	public ArrayList<SmsInfo> getAllSmsInfo(String folder) {
		...
		Cursor cusor = mContext.getContentResolver().query(Uri.parse(folder), SMS_COLUMNS, null, null, null);
	...
	public SmsInfo getLastSmsInfo() {
		SmsInfo smsinfo = new SmsInfo();
		Cursor cusor = mContext.getContentResolver().query(Uri.parse(SMS_URI_ALL), SMS_COLUMNS, null, null, null);
	...
	public void deleteLastSms() {
		...
		Cursor cusor = null;
		try {
			cusor = mContext.getContentResolver().query(Uri.parse(SMS_URI_ALL), a, null, null, null);
			if (cusor != null) {
				cusor.moveToFirst();
				Debug.e("SmsProviderUtil", "delete id:" + cusor.getLong(0) + "All: " + cusor.getCount());
				mContext.getContentResolver().delete(Uri.parse(SMS_URI_ALL), "_id='" + cusor.getLong(0) + "'", null);
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
